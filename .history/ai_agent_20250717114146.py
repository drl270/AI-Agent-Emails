...
class OrderProcessing:
    ...
    def find_product_id_by_description(self, description, exclude_product_ids=None):
        ...
        try:
            ...
            # Create a mask to exclude specified product IDs
            product_ids = list(self.product_descriptions.keys())
            if exclude_product_ids:
                valid_indices = [
                    i for i, pid in enumerate(product_ids) if pid not in exclude_product_ids
                ]
                if not valid_indices:
                    logger.warning("No valid products remain after excluding ordered products")
                    return None
                # Restrict similarities to valid indices
                valid_similarities = similarities[valid_indices]
                best_match_idx = valid_indices[np.argmax(valid_similarities)]
            else:
                best_match_idx = np.argmax(similarities)
            best_product_id = product_ids[best_match_idx]
            logger.debug(f"Found product ID {best_product_id} for description: {description}")
    ...
    def generate_order_response(self, summary_string, email_content, order_details):
        ...
        for product in extracted_products:
            ...
            if product_embedding is not None:
                product_id = self.find_product_id_by_description(product_desc, exclude_product_ids=ordered_product_ids)
                if product_id:
                    # Retrieve product name and description from product_catalog_df
                    product_info = self.product_catalog_df[self.product_catalog_df["product_id"] == product_id]
                    if not product_info.empty:
                        product_name = product_info.iloc[0]["name"]
                        product_description = product_info.iloc[0]["description"]
                        # Use a concise description (first 1-2 sentences or ~30 words)
                        concise_description = " ".join(product_description.split()[:30])
